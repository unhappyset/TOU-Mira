...
public static class EndGameEvents
{
    ...
    public static void OnGameEnd(GameEndEvent @event)
    {
        ...
        {
            winType = 1;
            GameHistory.WinningFaction = $"<color=#{Palette.CrewmateBlue.ToHtmlStringRGBA()}>{TranslationController.Instance.GetString(StringNames.Crewmates)}</color>";
        }
        ...
        {
            winType = 2;
            GameHistory.WinningFaction = $"<color=#{Palette.ImpostorRed.ToHtmlStringRGBA()}>{TranslationController.Instance.GetString(StringNames.ImpostorsCategory)}</color>";
        }
