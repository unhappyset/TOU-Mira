...
{
    public override string GroupName => TouLocale.Get(Medic, "Medic");
    ...
    public ModdedNumberOption MedicReportNameDuration { get; } = new($"Time Where {TouLocale.Get(Medic, "Medic")} Will Have Name", 0f, 0f, 60f,
        2.5f, MiraNumberSuffixes.Seconds)
    {
        Visible = () => OptionGroupSingleton<MedicOptions>.Instance.ShowReports
    };

    public ModdedNumberOption MedicReportColorDuration { get; } = new($"Time Where {TouLocale.Get(Medic, "Medic")} Will Have Color Type", 15, 0f,
